...
- **기술 스택**: PRD에 명시된 기술 요구사항 반영. 로드맵에 **버전 숫자를 적어야 할 경우**, 기존 프로젝트면 `package.json`의 실제 값을, 신규 프로젝트면 context7 MCP(`mcp__context7__resolve-library-id` → `mcp__context7__query-docs`)로 조회한 최신 안정 버전을 사용합니다. 기억에 의존해 버전을 적지 마세요
