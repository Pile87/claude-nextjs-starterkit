...
이 문서는 기술 스택의 버전을 고정하지 않습니다. PRD를 작성하기 **전에** 아래 순서로 실제 버전을 확인한 뒤 기입합니다.

1. **기존 프로젝트 위에서 실행 중이라면 — `package.json` 우선**: `dependencies` / `devDependencies`의 실제 설치 버전이 단일 진실 공급원입니다.
2. **신규 프로젝트라면 — context7 MCP로 최신 안정 버전 조회**: `mcp__context7__resolve-library-id`로 라이브러리 ID를 해석한 뒤 `mcp__context7__query-docs`로 현재 최신 안정 버전과 권장 사용법을 확인합니다.
   - 조회 대상: Next.js, React, TypeScript, TailwindCSS, shadcn/ui 등 PRD에 기재할 모든 핵심 스택
3. **기억에 의존한 버전 기입 금지**: 학습 시점 이후 메이저 버전이 올라갔을 수 있습니다. 버전 숫자는 반드시 위 1~2의 조회 결과에서 가져옵니다.
4. **버전 종속 설명 금지**: "설정 파일 없는 새로운 엔진" 같이 특정 버전에서만 참인 부연은 적지 않습니다. 다음 메이저에서 틀린 설명이 됩니다.
5. **폴백**: context7을 사용할 수 없으면 버전 숫자를 임의로 적지 말고 "설치 시점 최신 안정 버전"으로 표기하며, 확인하지 못했다는 사실을 PRD에 명시합니다.
...
- 상세한 기술 스택과 용도별 분류
- **반드시 최신 버전 명시**: 버전 숫자는 「버전 원칙」에 따라 `package.json` 또는 context7 조회 결과에서 가져옵니다 (기억으로 적지 않음)
...
[아래 `{...}` 자리에는 「버전 원칙」에 따라 확인한 실제 버전 숫자를 채워 넣습니다.
플레이스홀더를 그대로 출력하지 말고, 조회한 값으로 치환한 최종 문서를 생성합니다.]

### 🎨 프론트엔드 프레임워크

- **Next.js {확인한 버전}** (App Router) - React 풀스택 프레임워크
- **TypeScript {확인한 버전}** - 타입 안전성 보장
- **React {확인한 버전}** - UI 라이브러리

### 🎨 스타일링 & UI

- **TailwindCSS {확인한 버전}** - 유틸리티 CSS 프레임워크
- **shadcn/ui** - 고품질 React 컴포넌트 라이브러리
- **Lucide React** - 아이콘 라이브러리

### 📝 폼 & 검증

- **React Hook Form {확인한 버전}** - 폼 상태 관리
- **Zod {확인한 버전}** - 스키마 검증 라이브러리
...
6. **최신 기술**: 「버전 원칙」에 따라 확인한 **현재 최신 안정 버전** 명시 (기억이 아닌 조회 결과 기준)

## 🔧 기술 스택 선택 원칙

- **최신 버전 필수**: 버전은 「버전 원칙」의 조회 결과를 따릅니다
- **Next.js 기반**: App Router를 기본으로 하는 풀스택 구성
- **TailwindCSS**: 유틸리티 우선 스타일링 (설정 방식은 확인한 버전의 공식 문서를 따름)
- **TypeScript**: 타입 시스템으로 코드 안정성 확보
...
- PRD 작성 전 「버전 원칙」의 확인 절차를 먼저 수행할 것 (`package.json` 또는 context7)
- 모든 버전 숫자는 조회 결과에서 가져올 것 — 기억이나 관례로 적지 말 것
- 특정 버전에서만 참인 부연 설명(예: "설정 파일 없는 방식")은 붙이지 말 것
- 확인하지 못한 항목은 버전을 비우고 "설치 시점 최신 안정 버전"으로 표기할 것
...
7. 필요 데이터 모델 최소화
8. **「버전 원칙」에 따라 버전 확인 후** Next.js 기반 기술 스택 적용
9. **정합성 검증 체크리스트 실행**
10. 템플릿 형식으로 출력
