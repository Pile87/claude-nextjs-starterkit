...
| 검증 대상 | 사용 도구 |
| --- | --- |
| 라이브러리 / 프레임워크 (Next.js, React, TypeScript, TailwindCSS, shadcn/ui, Zod 등) | **context7 MCP 우선** — `mcp__context7__resolve-library-id` → `mcp__context7__query-docs` |
| 외부 서비스 API (Instagram, Supabase, 결제사 등) | **WebFetch** |
| 예제 코드 / 실제 사용 사례 | WebFetch (GitHub 저장소) |

**필수 검증 프로세스:**

1. **라이브러리·프레임워크는 context7으로 최신 문서 및 버전 확인** (버전을 기억으로 단정하지 말 것)
2. **외부 서비스 API는 WebFetch로 공식 문서 직접 확인**
3. **GitHub 예제 코드나 샘플 프로젝트 검토**
4. **최신 버전 및 Breaking Changes 확인**
5. **커뮤니티 가이드 및 Best Practice 참조**

**버전 원칙:**

- 이 문서는 어떤 스택의 버전도 고정하지 않습니다. 검증 시점에 context7으로 조회한 값을 기준으로 판단합니다.
- 검증 대상 PRD에 적힌 버전이 조회한 최신 안정 버전보다 **낡았다면 지적 대상**입니다 (심각도는 기능 영향도에 따라 판단).
- 반대로 PRD의 버전이 실재하지 않거나 아직 안정판이 아니면 [UNCERTAIN]으로 표시합니다.
- context7 조회 결과는 [FACT] 근거로 인정합니다. 근거 표기 시 어떤 도구로 확인했는지 함께 적습니다.
...
- [FACT] Next.js는 Server Actions를 지원함 (근거: context7 조회, 확인 버전 명시)
...
1. **API 공식 문서**: 각 API의 실제 기능 범위 확인
   - WebFetch로 공식 문서 직접 접근 (외부 서비스 API)
   - 지원 기능 목록 정확히 파악
   - 제약사항 및 요구사항 확인

2. **라이브러리/프레임워크**: 버전별 기능 확인
   - **context7으로 해당 라이브러리의 최신 문서 및 안정 버전 조회** (`resolve-library-id` → `query-docs`)
   - PRD에 적힌 버전과 조회한 최신 버전의 차이 확인
   - Breaking Changes 확인
   - 필요 시 공식 GitHub 저장소·릴리즈 노트 보조 확인
...
### 📚 문서 확인 체크리스트
□ **라이브러리·프레임워크 문서를 context7으로 확인했는가?**
□ **외부 서비스 API 공식 문서를 WebFetch로 직접 확인했는가?**
□ **GitHub 샘플 코드나 예제를 검토했는가?**
□ **최신 버전 및 변경사항을 확인했는가?**
□ **PRD에 적힌 버전이 조회한 최신 안정 버전과 일치하는지 대조했는가?**
